...
function formatFacilityAddress(evidence) {
  return concatenateReadableString(
...
function formatId(identity) {
  ...
  if (identity.includes("aadhaar") && lastFragment.length >= 4) {
    return (
      "Aadhaar # XXXX XXXX XXXX " + lastFragment.substr(lastFragment.length - 4)
    );
  }
...
async function createCertificatePDF(certificateResp, res, source) {
  if (certificateResp.length > 0) {
    ...
    const zippedData = await zip
      .generateAsync({ type: "string", compression: "DEFLATE" })
    ...
    const {
      certificate: { credentialSubject, evidence },
    } = certificateRaw;
    const certificateData = {
      ...
      vaccinationDate:
        formatDate(evidence[0].date) + ` (Batch no. ${evidence[0].batch} )`,
      vaccineValidDays: `after ${getVaccineValidDays(
        evidence[0].effectiveStart,
        evidence[0].effectiveUntil
      )} days`,
    ...
    const htmlData = fs.readFileSync(
      `${__dirname}/../../configs/templates/certificate_template.html`,
      "utf8"
    );
...
async function createCertificatePDFByCertificateId(phone, certificateId, res) {
  const certificateResp = await registryService.getCertificate(
    phone,
    certificateId
  );
...
async function createCertificatePDFByPreEnrollmentCode(preEnrollmentCode, res) {
  const certificateResp =
    await registryService.getCertificateByPreEnrollmentCode(preEnrollmentCode);
...
async function getCertificate(req, res) {
  try {
    ...
    const certificateId = req.url
      .replace("/certificate/api/certificate/", "")
      .split("?")[0];
    res = await createCertificatePDFByCertificateId(
      claimBody.Phone,
      certificateId,
      res
    );
...
async function getCertificatePDFByPreEnrollmentCode(req, res) {
  try {
    ...
    try {
      claimBody = await verifyKeycloakToken(req.headers.authorization);
      preEnrollmentCode = req.url.replace(
        "/certificate/api/certificatePDF/",
        ""
      );
    } catch (e) {
      console.error(e);
...
async function checkIfCertificateGenerated(req, res) {
  try {
    ...
    const certificateResp =
      await registryService.getCertificateByPreEnrollmentCode(
        preEnrollmentCode
      );
    if (certificateResp.length > 0) {
      res.statusCode = 200;
