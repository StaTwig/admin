...
const ViewShippingModal = (props) => {
  const { singleShippingOrder } = props;

  let totalQuantity = 0;
  singleShippingOrder.products.forEach(
  ...
  return (
    <div className='PO'>
      ...
      <table className='table poModalTable'>
        ...
        <tbody>
          {singleShippingOrder.products.map((product) => (
            <tr>
              <th scope='row'>
      ...
      <Link to={`/newshipment?shippingId=${singleShippingOrder.id}`}>
        <button className='btn btn-orange fontSize20 font-bold mr-2  mt-3 float-right'>
          <img
            src={Order}
            width='14'
            height='14'
