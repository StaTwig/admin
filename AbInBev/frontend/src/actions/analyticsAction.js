...
export const getOrgTypeStats = (param) => {
	return async (dispatch) => {
		try {
			dispatch(turnOn());
			const result = await axios.get(config().getOrganisationTypeStatsurl + param);
			dispatch(turnOff());
			return result.data;
		} catch (e) {
			dispatch(turnOff());
...
export const getSupplierPerformanceByOrgType = (data) => {
	return async (dispatch) => {
		...
		try {
			const result = await axios.get(config().getSupplierPerfomance + queryParam);
			return result.data;
		} catch (e) {
			dispatch(turnOff());
		}
	...
	return async (dispatch) => {
		try {
			...
			const result = await axios.get(
				config().getNewConfig + `?district=${data.district}&vendorType=${data.vendorType}`,
			); //BELGAUM S1
			dispatch(turnOff());
			return result.data;
		} catch (e) {
