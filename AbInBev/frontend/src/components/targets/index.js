...
const Targets = (props) => {
    ...
    const onStateChange = async (event) => {
        ...
        setDisplayDistricts(result.data)
        setParams(filter)
    }
                    ...
                    <div className="row">
                        {openTextMsg && (
                            <div>
                                <div className="uploadModel">
                                    <span className="closeBtn">New Targets have been updated for the selected Districts</span>
                                    <div>
                                        <button
                                            type="button"
                        ...
                        <div className="col-md-9 mainContainer pt-3 px-4">
                            <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center">
                                ...
                                <button className="saveBtn" onClick={() => { setOpenTextMsg(true) }}>Save</button>
                            </div>

                            <div className="titles">
                                <div className="distric-01">
                                    <input type="checkbox" id="districts" onChange={selectedAllDistricts} style={{ marginRight: "5px" }} />
                            ...
                            <div className="totalBox" style={{ padding: `${displayDistricts.length > 0 ? '2rem' : 'unset'}` }}>
                                <div className="districtList">
                                    {displayDistricts.map((district, index) => (
                                        <div style={{ marginBottom: "20px" }}>
                                            {checkedAllDis ? (
                                                <input key={index} type="checkbox" style={{ marginRight: "5px" }} checked={checkedAllDis} />
                                            ) : (
                                                <input id={"distict" + index} onChange={selectCheckBox} type="checkbox" style={{ marginRight: "5px" }} />
                                            )}
                                ...
                                <div className="returTarget">
                                    {displayDistricts.map((district, index) => (
                                        <span style={{ marginBottom: "20px" }}>50%</span>
                                    ))}
                                </div>
                                <div className="setReturnTarget">
                                    ...
                                    {displayDistricts.map((district, index) => (
                                        <div style={{ marginBottom: "20px" }}>
                                            <select value="" style={{ borderRadius: "10px" }}>
                                                <option key={index}>80%</option>
                                                {percentageList?.map((item, index) => (
                                                    <option key={index}>{item.value}</option>
                                                ))}
                                            </select>
                                            {/* // <span style={{ marginBottom: "20px" }}>80%</span> */}
                                        </div>
                                    ))}
                        ...
                        <div className="col-md-3 rightSideMenu pt-4 px-2">
                            <div className="filterSection">
                                ...
                                <div>
                                    ...
                                    <label className="filterSubHeading mt-3" style={{ opacity: `${!disableReturnTarget ? "unset" : "0.5"}` }}>
                                    ...
                                    >
                                        ...
                                        {percentageList?.map((item, index) => (
                                            <option key={index} value={item}>{item.value}</option>
                                        ))}
                                    </select>
                                </div>
