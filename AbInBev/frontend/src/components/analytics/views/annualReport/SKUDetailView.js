...
const SKUDetailView = (props) => {
	...
	useEffect(() => {
		(async () => {
			...
			if (props.params) {
				...
				if (props.params?.year) qry += "&year=" + props.params?.year;
	...
	const getAnalyticsByType = async (district, i) => {
		...
		const result = await dispatch(
			getOrgTypeStats(
				"?sku=" + (props.sku ? props.sku : prop.externalId) + "&district=" + district,
			),
		);
						...
						<div className="productDetailCard">
							<div className="productGrid">
								<img
									...
									src={brandsIconArr[brands.indexOf(prop.manufacturer.split(" ").join(""))]}
								/>
							</div>
								...
								<div className="row">
									<div className="col-lg-6 col-md-6 col-sm-12">
										<div className="productSection mb-2">
											<div className="profile">
												<img
													src={brandsArr[brands.indexOf(prop.manufacturer.split(" ").join(""))]}
													alt=""
													height="60"
												/>
											</div>
									...
									<div className="col-lg-6 col-md-6 col-sm-12">
										...
										<div className="captionSubtitle">
											Compared to ({!isNaN(prop.returnRatePrev) ? prop.returnRatePrev : 0}% last
											month)
										</div>
										<div className="progress progress-line-default">
											<div
												...
												aria-valuemax="100"
												style={{
													width: (!isNaN(prop.returnRate) ? prop.returnRate : 0) + "%",
												}}
											>
				...
				<div className="row">
					<div className="col-md-12 col-sm-12">
						<div className="productsChart">
							{isActive && <h2 className="stateTitle">{stateLabel} SKU Details</h2>}

							<ResponsiveContainer width="100%" height={analytics.length <= 1 ? 300 : 1500}>
								<BarChart
									width={500}
									height={300}
									data={analytics}
								...
								>
									...
									<Bar
										...
										radius={[0, 5, 5, 0]}
									/>
									<Bar name="Returns" dataKey="returns" fill="#A20134" stackId="b" />
									<Bar
										name="Target Returns"
										dataKey="targetSales"
										fill="#A344B7"
													...
													<tr key={index}>
														<td scope="row">
															<span
																className="stateLink"
																onClick={() => {
																	if (isActive) getAnalyticsByType(analytic.groupedBy, index);
																	else {
