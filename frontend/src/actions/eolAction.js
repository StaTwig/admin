  ...
  return async (dispatch) => {
    try {
      ...
      const result = await axios.get(
        `${
          config().GetEOLInfo
        }?skip=${skip}&limit=${limit}&country=${country}&state=${state}&city=${district}&location=${location}&product=${product}`
      );
...
export const GetEOLInfoByPlaceAdministered = async (data) => {
  try {
    const result = await axios.get(
      config().GetEOLInfoByPlaceAdministered + data
    );
    return result.data;
  } catch (e) {
    return [];
