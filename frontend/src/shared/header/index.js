...
const Header = (props) => {
		...
		if (search.substring(0, 2) === "SH") {
			...
			axios
				...
				.then((resp) => {
					...
					else alert(`${t("the_product")} "${searchValue}" ${t("not_found_in_inventory")}`);
				})
				.catch((err) => {
			...
			axios
				...
				.then((resp) => {
					if (resp.data.data.length > 0) props.history.push(`/productinventory/${searchValue}`);
					else alert(`${t("there_no_products_type")} "${searchValue}" ${t("in_your_inventory")}`);
				})
	...
	useEffect(() => {
		...
		async function fetchApi() {
			...
			setNotifications(response.data.data?.data?.reverse());
			if (response.data?.data?.totalNew) setNewNotifs(response.data?.data?.totalNew);
			if (response.data?.data?.totalUnRead) setNewNotifs(response.data?.data?.totalUnRead);
							...
							<li className="navItems">
								<div className="search-form" tabIndex="-1" onKeyDown={onkeydown}>
									<Autocomplete
										...
										onBlur={(e) => (e.target.placeholder = { search_placeholder })}
						...
						<li className="navItems notifyList">
							<div className="notifications cursorP">
								...
								<div
									...
									onClick={() => setShowNotifications(!showNotifications)}
								>
									{notifications?.length && <span className="badge badge-light">{newNotifs}</span>}
								</div>
								...
								{showNotifications && (
									...
									>
										<div
											className="nheader"
											style={{
												backgroundImage: "linear-gradient(to right, #0092e8, #0a6bc6)",
											}}
										>
											<div className="user-notification-head">{t("user_notification")}</div>
											...
											<div className="noti-tab">
												<ul className="nav nav-pills">
													<li
														className={visible === "one" ? "nav-item-active" : "nav-item"}
														onClick={() => {
															setLimit(10);
															setAlertType("ALERT");
													...
													>
														<div className={visible === "one" ? "nav-link" : "nav-link tab-text"}>
															{t("alerts")}
														</div>
													</li>
													<li
												...
												{notifications?.length >= 0 ? (
													notifications?.map((notifications) =>
														notifications.transactionId ? (
															notifications.eventType !== "REQUEST" ? (
																...
																>
																	...
																	<div className="text-secondary notif-time">
																		{formatDistanceToNow(
																			new Date(
																				parseInt(notifications._id.toString().substr(0, 8), 16) *
																					1000,
																			),
																		)}
																	</div>
															...
															<div key={notifications.id} style={{ cursor: "not-allowed" }}>
																<div className="col-sm-10" style={{ display: "flex" }}>
																	<img
																		className="notification-icons"
																		src={notifIcon(notifications)}
																		alt="Icon"
																	/>
																	<div className="notification-events">{notifications.message}</div>
																</div>
																<div className="text-secondary notif-time">
																	{formatDistanceToNow(
																		new Date(
													...
													<div className="slider-item-no-notify" style={{ overflow: "hidden" }}>
														<div className="row" style={{ margin: "0 !important" }}>
															<div className="col text-center mt-3 mr-5">
																<div style={{ overflow: "hidden !important" }}>
																	<span className="no-notification">{t("no_notifications")}</span>
																</div>
															</div>
														</div>
													</div>
