    ...
    <>
     <div className={`row orderTxt ml-${index} mr-0 pl-3`}>
      ...
      <div className={`pt-${index} p-0 col ${pindex > 1 && `border-primary border-left`} `}>
        ...
        <div className={`panel row ml-${index} container-${container} mr-0  commonpanle`}>
          {data.poUpdates.map((r, i) =>
            <div key={i} className="col-12 row justify-content-between">
              <div className={`${op >= index && index !== 0 ? `col-5` : `col-10`}`}>
              ...
              <div className="text-primary col-2">
                ...
                <div className="text-muted">{formatTimeAMPM(r.updatedOn.split(' ')[1])}</div>
              </div>
            </div>
          )}
          {op >= index && index !== 0 &&
            <>
              ...
              {data.products.map((product, index) => 
                <div key={index} className="col-12 row justify-content-between">
                  ...
                  <span className="text-muted text-weight-bold">{product.quantity}</span>
                </div>
              )}
              </div>
        ...
        {op >= index && index !== 0 ?
          <div className="arrow bg-primary float-right" onClick={() => { setOp(-1); setVisible(false);}}>
            <img src={Down} alt="actions" height="7" width="12" />
          </div>
        :
          <div className="arrow float-right" onClick={() => { setOp(1); setVisible(false); }}>
