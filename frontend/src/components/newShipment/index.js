              ...
              {products?.length > 0 && (
                ...
                <EditTable
                  ...
                  handleQuantityChange={(v, i) => {
                    ...
                    setFieldValue('products', newArr.map(row => ({"ProductCategory":row.prodtype,"productID": row._id,"productQuantity": row.productQuantity,"productName": row.productName,"manufacturer": row.manufacturer,"quantity": row.quantity})));
                    setAddProducts(prod => [...newArr]);
                  }}
                  enableDelete={true}
                  onRemoveRow={(index) => {
