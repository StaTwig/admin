			...
			{addresses.map((address, index) => (
				<div
					...
					style={{
						...
						cursor: "pointer",
						position: "relative",
						backgroundColor: `${props.selectedAddress.warehouse === address.id ? "#DFF1F2" : ""}`,
						left: `${props.addresses.length < 2 ? `3rem` : `unset`}`,
					}}
                 ...
                 */}
					<div className="addressalign">
						<span className="title">{address.title}</span>
						<br />
						<span className="secondAddress">
							{address.warehouseAddress.city}, {address.warehouseAddress.state},{" "}
							{address.warehouseAddress.country}
						</span>
						<br />
						<span>
							{address.warehouseAddress.firstLine}, <br />
