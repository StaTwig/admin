...
const NUModal = (props) => {
	...
	const unDisableNxtBtn = () => {
		...
		if (props.data.id && disableRoleBtn) {
			...
			if (userAlreadyExits) return true;
			else if (disableRoleBtn && disableButton) return false;
			else return true;
		}
	};
			...
			>
				{({
					...
					<form onSubmit={handleSubmit}>
						<p className="pl-4 pt-2 txtColor" style={{ marginTop: -25, fontSize: 14 }}>
							{t(
								"Enter the email address of the users you'd like to add or invite and choose the role they should have.",
							)}
						</p>
						{props?.isAddNewUser && (
							<div
								className="pl-4 pr-4 pt-3 d-flex pb-4"
							...
							>
								<div className="input-group" style={{ width: "49.5%", alignItems: "center" }}>
									<input
										...
										className={`form-control ${
											errors.first_name && touched.first_name ? "border-danger" : ""
										}`}
								...
								<div className="input-group" style={{ width: "49.5%", alignItems: "center" }}>
									<input
										...
										id="last_name"
										className={`form-control ${
											errors.last_name && touched.last_name ? "border-danger" : ""
										}`}
              ...
              <div className="input-group" style={{ width: '45%', alignItems: 'center' }}>
                  ...
                  />
						...
						<div
							className="pl-4 pr-4 pt-3 d-flex pb-4 shadow"
							style={{ justifyContent: "space-between" }}
						>
							<div className="input-group" style={{ width: "45%", alignItems: "center" }}>
								<input
							...
							>
								...
								<span className="txt-btn" style={{ fontSize: "12px" }}>
									{t("add_new_user_role")}
								</span>
							</button>
						</div>
						...
						>
							{changeComponent === "role" ? (
								<div>
									{props?.defaultRoles && props?.defaultRoles.length !== 0
										? props?.defaultRoles.map((per, index) => {
												return (
													<Role
														key={index}
														title={per.role}
									...
									{errors.role && touched.role && (
										<span className="pl-3 error-msg text-danger">{errors.role}</span>
									)}
								</div>
							) : (
								<div
									style={{
										...
										flexDirection: "row",
										flexWrap: "wrap",
										justifyContent: `${addresses.length < 2 ? `unset` : `space-evenly`}`,
									}}
								>
						...
						<div className="d-flex flex-row-reverse p-3">
							{changeComponent === "role" ? (
								<button
									...
									onClick={() => {
										setChangeComponent("address");
										setButtonText("ADD USER");
										scrolling.current.scrollTop = 0;
									}}
								...
								<button
									...
									className="ml-3 btn btn-orange"
									disabled={addUserBtnDisable || userAlreadyExits}
								>
									{t(buttonText)}
								</button>
							)}
