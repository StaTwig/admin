...
const CreateShippingOrder = (props) => {
  ...
  let totalQuantity = 0;
  props.purchaseOrder.products.forEach(
    (product) => (totalQuantity += parseInt(product.productQuantity))
  );

  useEffect(() => {
    async function fetchData() {
      const result = await getWarehouseByOrgId(
        props.purchaseOrder.suppplierOrgID
      );
  ...
  return (
    <div className='createshipping'>
        ...
        <div className='input-group'>
          ...
          <div className='form-control'>
            <DropdownButton
              ...
              onSelect={(item) => {
                setWareHouseId(item);
                const location = warehouses.find(
                  (w) => w.id === item
                ).postalAddress;
      ...
      {openCreatedShipping && (
        ...
        >
          <AlertModal onHide={closeModal} {...modalProps} {...props} />
        </Modal>
      )}
      <hr />
      ...
      <button
        className='btn btn-orange fontSize20 font-bold mr-2 float-right'
        onClick={onCreateShippingOrder}
      >
        <img src={Order} width='14' height='14' className='mr-2' alt='Order' />
