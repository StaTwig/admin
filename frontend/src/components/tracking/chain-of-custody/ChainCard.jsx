						...
						<div className="accordion-header">
							<div className="accordion-info">
								<div className="content-info-card">
									...
									<p className="mi-body-sm f-500 mi-reset">Order ID: {shipmentData?.id}</p>
								</div>
								<div className="content-info-card">
									<div className="content-icon-space">
									...
									<p className="mi-body-xs f-500 mi-reset grey">
										{shipmentData?.actualDeliveryDate ? (new Date(shipmentData?.actualDeliveryDate).toLocaleDateString()) : "N/A"}
									</p>
								</div>
							</div>
							...
							<div className="product-details-list">
								{shipmentData?.products?.map((product, index) => (
									<div key={index} className="product-list-card">
										...
										<p className="mi-body-sm f-500 mi-reset">{`${product?.productQuantity} (${product?.unitofMeasure?.name})`}</p>
									</div>
								))}
							</div>
							<button className="mi-btn mi-btn-sm mi-btn-secondary" onClick={() => redirectToShipment(shipmentData?.id)} >View Order</button>
						</div>
					</AccordionDetails>
				</Accordion>
			</div>
