...
exports.getOverviewStats = [
	...
	async function (req, res) {
		try {
			...
			filters.type =
				req.query.orgType && req.query.orgType.length ? req.query.orgType : BREWERY_ORG;
			const resPerPage = 10;
			...
			return apiResponse.successResponseWithData(res, "Operation success", finalData);
		} catch (err) {
...
exports.getAllBrands = [
	...
	async function (req, res) {
		try {
			...
			return apiResponse.successResponseWithData(res, "Operation success", allBrands);
		} catch (err) {
			return apiResponse.ErrorResponse(res, err.message);
		}
	},
		...
		try {
			...
			for (const [index, product] of Products.entries()) {
				...
				let prods = MasterProducts.filter(
					(prod) =>
						prod.externalId == product._id.id && prod.manufacturer == product._id.manufacturer,
				);
				if (prods.length) {
					for (const [i, prod] of prods.entries()) {
						...
						if (analyticsFilter?.uploadDate) to = analyticsFilter.uploadDate["$lte"];
						if (analyticsFilter?.uploadDate) from = analyticsFilter.uploadDate["$gte"];
			...
			if (!bool) {
				return apiResponse.successResponseWithData(res, "Operation success", Analytics);
			}
		...
		try {
			...
			for (let analytic of Analytics) {
				...
				products.forEach((product) => {
					...
					targetSalesTotal =
						targetSalesTotal + (parseInt(product.targetSales) ? parseInt(product.targetSales) : 0);
					returnsTotal = returnsTotal + (parseInt(product.returns) ? parseInt(product.returns) : 0);
					returnRateTotal =
						returnRateTotal + (parseInt(product.returnRate) ? parseInt(product.returnRate) : 0);
					returnRatePrevTotal =
						returnRatePrevTotal +
						(parseInt(product.returnRatePrev) ? parseInt(product.returnRatePrev) : 0);
				});
...
exports.getLeadTimes = [
	...
	async function (req, res) {
		try {
			...
			let shipmentLeadTimes = await ShipmentModel.aggregate([
				...
				},
				{
					$project: {
						...
						id: 1,
						leadtime: {
							$divide: [{ $subtract: ["$actualDeliveryDate", "$shippingDate"] }, 60 * 1000 * 60],
						},
					},
				},
				{
			...
			return apiResponse.successResponseWithData(res, "Operation success", shipmentLeadTimes);
		} catch (err) {
			return apiResponse.ErrorResponse(res, err.message);
		}
	},
...
async function calculateDirtyBottlesAndBreakage(supplierOrg) {
	try {
		...
		let shipmentUpdates = shipments.filter(
			(sh) => sh.shipmentUpdates.filter((s) => s.updateComment === "Receive_comment_3").length,
		);
		...
		let damagedShipments = shipments.filter(
			(sh) =>
				sh.shipmentUpdates.filter(
					(s) => s.updateComment === "Receive_comment_1" || s.updateComment === "Receive_comment_2",
				).length,
		);
		...
		return {
			dirtyBottles: dirtyBottles > 0 ? parseFloat(dirtyBottles / totalProductd).toFixed(2) : 0,
			breakage: breakage > 0 ? parseFloat(breakage / totalProductb).toFixed(2) : 0,
		};
	} catch (err) {
		console.log(err);
	}
...
exports.getSupplierPerformance = [
	...
	async function (req, res) {
		try {
			...
			if (!bool) {
				return apiResponse.successResponseWithData(res, "Operation success", supplierOrgs);
			}
		} catch (err) {
					...
					[
						{
							$set: {
								targetSales: {
									$divide: [
										{
											$multiply: ["$sales", parseInt(uniqueTarget.percentage, 10)],
										},
										100,
									],
								},
...
exports.getAllConfiguration = [
	...
	async function (req, res) {
		try {
			...
			return apiResponse.successResponseWithData(res, "Operation success", configuration);
		} catch (err) {
			return apiResponse.ErrorResponse(res, err.message);
		}
	},
