...
async function getOverallRating(rating) {
  try {
    ...
    if (rating) {
      ...
      overallRating += rating.bottleCapacityRating
        ? rating.bottleCapacityRating * 0.2
        : 0;
      overallRating += rating.dirtyBottlesRating
        ? rating.dirtyBottlesRating * 0.2
        : 0;
      ...
      if (schema) {
        let temp =
          value / schema.target < 1
            ? (value / schema.target) *
                (schema.max.rating - schema.min.rating) +
              schema.min.rating
...
async function getSupplierTargets(rating) {
  try {
    ...
    if (rating) {
      targets.returnRateTarget = rating.returnRate?.target
        ? rating.returnRate.target
        : "N/A";
      targets.leadTimeTarget = rating.leadTime?.target
        ? rating.leadTime.target
        : "N/A";
      targets.storageCapacityTarget = rating.bottleCapacity?.target
        ? rating.bottleCapacity.target
        : "N/A";
      targets.dirtyBottlesTarget = rating.dirtyBottle?.target
        ? rating.dirtyBottle.target
        : "N/A";
      targets.breakageTarget = rating.breakageBottle?.target
        ? rating.breakageBottle.target
        : "N/A";
    }
    return targets;
  } catch (err) {
...
function getConditionsOrgWarehouse(filters) {
  try {
    ...
    if (filters.district && filters.district.length) {
      matchCondition["warehouseDetails.warehouseAddress.city"] =
        filters.district;
    }
