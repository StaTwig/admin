  ...
  exports.getCountryDetailsByRegion = [
    ...
    async (req, res) => {
      try {
        ...
        if(region && orgType) {
          var countries = await CountryModel.find({region: region},{'_id': 0}).select('name');
          countries = countries.map(country => country.name);
          const orgs = await OrganisationModel.find({type:orgType}).select('country id');
          try{
            const warehouseCountries = await WarehouseModel.find({organisationId :{$in: orgs.map(org => org.id)}}).select('country');
    ...
    auth,
      async (req, res)=>{
          try {
              ...
              return apiResponse.successResponseWithData(res,`Organizations of type ${orgType} & country ${country}`, orgs);
          } catch (err) {
              return apiResponse.ErrorResponse(res, err.message);
          }
      }
