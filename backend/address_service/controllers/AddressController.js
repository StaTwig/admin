	...
	async (req, res) => {
		try {
			await Warehouse.find({
				$and: [{ organisationId: req.user.organisationId }, { status: "ACTIVE" }],
			})
				.then((warehouses) => {
	...
	async (req, res) => {
		try {
			...
			const warehouses = await Warehouse.find({
				$and: [{ organisationId: req.query.orgId }, { status: "ACTIVE" }],
			});
	...
	async (req, res) => {
		...
		try {
			...
			await Warehouse.aggregate([
				{
					$match: getConditionForLocationApprovals(orgType, req.user.organisationId),
				},
				{
					$lookup: {
						from: "employees",
				...
				.then((warehouses) => {
					return apiResponse.successResponseWithData(res, "Warehouses details", warehouses);
				})
				.catch((err) => {
...
exports.updateAddressOrg = [
	...
	async (req, res) => {
		try {
			await Organisation.findOneAndUpdate({ id: req.user.organisationId }, req.body.address, {
				new: true,
			})
				.then((org) => {
...
exports.updateWarehouseAddress = [
	...
	async (req, res) => {
		try {
			await Warehouse.findOneAndUpdate({ id: req.query.warehouseId }, req.body.WarehouseAddress, {
				new: true,
			})
				.then((warehouse) => {
...
exports.AddOffice = [
	...
	async (req, res) => {
		try {
			...
			await office.save();
			return apiResponse.successResponseWithData(res, "Office added success", office);
		} catch (err) {
			return apiResponse.ErrorResponse(res, err);
		}
	},
...
exports.modifyLocation = [
	...
	async (req, res) => {
		try {
			...
			await Warehouse.updateOne({ id: id }, { status: type === 1 ? "ACTIVE" : "REJECTED" })
...
exports.getStatesByCountry = [
	...
	async (req, res) => {
		try {
			...
			return apiResponse.successResponseWithData(res, "Operation success", allStates);
		} catch (err) {
			return apiResponse.ErrorResponse(res, err.message);
		}
	},
...
exports.getCitiesByState = [
	...
	async (req, res) => {
		try {
			...
			return apiResponse.successResponseWithData(res, "Operation success", allCities);
		} catch (err) {
			return apiResponse.ErrorResponse(res, err.message);
		}
	},
