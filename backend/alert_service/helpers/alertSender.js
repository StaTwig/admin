...
async function shipmentMessage(event, txnId) {
  if (event.eventTypePrimary == "CREATE")
    return `"Shipment - ${txnId}" has been Created`;
  else if (event.eventTypePrimary == "UPDATE")
    return `"Shipment - ${txnId}" has been Updated`;
  else if (event.eventTypePrimary == "RECEIVE")
    return `"Shipment - ${txnId}" has been Delivered`;
  else return `"Shipment - ${txnId}" has been Updated`;
}

async function inventoryMessage(event, txnId) {
  if (event.eventTypePrimary == "ADD")
    return `"Inventory - ${txnId}" has been Created`;
  else if (event.eventTypePrimary == "EXPIRED")
    return `"Inventory - ${txnId}" has expired`;
  else if (event.eventTypePrimary == "NEAR_EXPIRY")
    return `"Inventory - ${txnId}" is nearing Expiry`;
...
async function orderMessage(event, txnId, actorOrgId) {
  ...
  if (event.eventTypePrimary == "RECEIVE")
    return `Received a new Order "Order - ${txnId}" from ${actorOrgId} (${organization?.name})`;
  else if (event.eventTypePrimary == "ACCEPT")
    return `Your "Order ID: ${txnId}" has been Accepted by ${actorOrgId} (${organization?.name})`;
  else if (event.eventTypePrimary == "REJECT")
    return `Your "Order ID: ${txnId}" has been Rejected by ${actorOrgId} (${organization?.name})`;
  else if (event.eventTypePrimary == "CREATE")
    return `"Order ID: ${txnId}" has been Created by ${actorOrgId} (${organization?.name})`;
...
async function alertMobile(event, mobile) {
  try {
    const content = await eventToData(event, "mobile");
    axios
      .post(process.env.MESSAGING_SERVICE_URL, {
        subject: "Testing otp",
        phone: mobile,
        otp: "0007",
        message: content,
